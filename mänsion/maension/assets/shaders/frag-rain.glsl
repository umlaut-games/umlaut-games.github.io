...
vec2 random_drop_pos(float val, vec2 screen_dim, vec2 velocity) {
    ...
    float y = (1.0 + 0.2 * random1d(1.234 * val)) * screen_dim.y;
...
vec3 wave_color(vec2 pixel, vec2 pos, float size, float time) {
    ...
    float inner_radius = (0.05 + 0.1 * time) * size;
...
vec3 background_color(vec2 pixel, vec2 screen_dim, float time) {
    return vec3(0.0, 0.0, 1.0 - smoothstep(-1.0, 0.0 + 0.2 * cos(0.1 * time), pixel.y / screen_dim.y));
}
...
void main() {
    ...
    vec2 velocity = vec2(u_mouse.x - 0.5 * dimensions.x, -0.9 * dimensions.y) / fall_time;
    ...
    for (float i = 0.0; i < n_drops; ++i) {
        ...
        float time = time + life_time * (i + i / n_drops);
        ...
        vec2 initial_pos = random_drop_pos(i + floor(time / life_time - i) * n_drops, dimensions, velocity);
